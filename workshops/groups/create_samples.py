    ...
    for item in local_folder.iterdir():
        if item.is_dir():
            new_box_folder = create_box_folder(
                client, item.name, box_base_folder
            )
            logging.info(" Folder %s", item.name)
            folder_upload(client, new_box_folder, item)
        else:
            file = file_upload(client, item, box_base_folder)
            logging.info(
                " \tUploaded %s (%s) %s bytes", file.name, file.id, file.size
            )
...
def file_upload(client: Client, file_path: str, folder: Folder) -> File:
    ...
    if file_id is None:
        # upload new file
        files: File = client.uploads.upload_file(
            upload_arg, file=open(file_path, "rb")
        )
        file = files.entries[0]
    else:
        # upload new version
        files: File = client.uploads.upload_file_version(
            file_id, upload_arg, file=open(file_path, "rb")
        )
        file = files.entries[0]
...
def create_samples(client: Client):
    ...
    # module_folder = create_box_folder(client, "users", wks_folder)
    docs_folder = create_box_folder(client, "groups", wks_folder)
    # create_box_folder(client, "requested_files", module_folder)

    folder_upload(client, docs_folder, "workshops/groups/content_samples/")
